...
import {DomainObjs} from "maci-contracts/sol/DomainObjs.sol";
import {IncrementalQuinTree} from "maci-contracts/sol/IncrementalQuinTree.sol";
import {IncrementalMerkleTree} from "maci-contracts/sol/IncrementalMerkleTree.sol";
import {SignUpGatekeeper} from "maci-contracts/sol/gatekeepers/SignUpGatekeeper.sol";
import {InitialVoiceCreditProxy} from "maci-contracts/sol/initialVoiceCreditProxy/InitialVoiceCreditProxy.sol";
import {SnarkConstants} from "maci-contracts/sol/SnarkConstants.sol";
import {ComputeRoot} from "maci-contracts/sol/ComputeRoot.sol";
import {MACIParameters} from "maci-contracts/sol/MACIParameters.sol";
import {VerifyTally} from "maci-contracts/sol/VerifyTally.sol";
...
contract MACI is DomainObjs, ComputeRoot, MACIParameters, VerifyTally {
    ...
    // Events
    event SignUp(PubKey _userPubKey, uint256 _stateIndex, uint256 _voiceCreditBalance);
    ...
    constructor(
        ...
        require(
            _maxValues.maxUsers <= stateTreeMaxLeafIndex || _maxValues.maxMessages <= messageTreeMaxLeafIndex,
    ...
    ) public isBeforeSignUpDeadline {
        if (signUpDurationSeconds == 0) {
            require(msg.sender == coordinatorAddress, "MACI: only the coordinator can submit signups in debug mode");
        }
     ...
     */
    function publishMessage(Message memory _message, PubKey memory _encPubKey) public isBeforeVotingDeadline {
        if (signUpDurationSeconds == 0) {
            require(msg.sender == coordinatorAddress, "MACI: only the coordinator can publish messages in debug mode");
        }
     ...
     */
    function genBatchUstPublicSignals(uint256 _newStateRoot, PubKey[] memory _ecdhPubKeys)
        public
        view
    ...
    function batchProcessMessage(
        ...
        require(hasUnprocessedMessages, ERROR_NO_MORE_MESSAGES);

        require(_ecdhPubKeys.length == messageBatchSize, ERROR_INVALID_ECDH_PUBKEYS_LENGTH);

        // Ensure that currentMessageBatchIndex is within range
        require(currentMessageBatchIndex <= messageTreeMaxLeafIndex, ERROR_CURRENT_MESSAGE_BATCH_OUT_OF_RANGE);

        // Assemble the public inputs to the snark
        uint256[] memory publicSignals = genBatchUstPublicSignals(_newStateRoot, _ecdhPubKeys);
        ...
        for (uint8 i = 0; i < publicSignals.length; i++) {
            require(publicSignals[i] < SNARK_SCALAR_FIELD, ERROR_PUBLIC_SIGNAL_TOO_LARGE);
        }
        ...
        // Verify the proof
        require(batchUstVerifier.verifyProof(a, b, c, publicSignals), ERROR_INVALID_BATCH_UST_PROOF);
    ...
    function proveVoteTallyBatch(
        ...
        require(currentQvtBatchNum < totalBatches, ERROR_ALL_BATCHES_TALLIED);
    ...
    function verifyTallyResult(
        ...
        uint256 computedRoot = computeMerkleRootFromPath(_depth, _index, _leaf, _pathElements);
     ...
     */
    function verifySpentVoiceCredits(uint256 _spent, uint256 _salt) public view returns (bool) {
