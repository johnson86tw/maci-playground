...
describe("MACI factory", () => {
  ...
  beforeEach(async () => {
    ...
    expect(await getGasUsage(maciFactory.deployTransaction)).lessThan(10000000); // origin: 5600000
  ...
  it("sets MACI parameters", async () => {
    ...
    await expect(maciFactory.setMaciParameters(...maciParameters.values())).to.emit(
      maciFactory,
  ...
  it("does not allow to decrease the vote option tree depth", async () => {
    ...
    await expect(maciFactory.setMaciParameters(...maciParameters.values())).to.be.revertedWith(
  ...
  it("allows only owner to set MACI parameters", async () => {
    ...
    await expect(coordinatorMaciFactory.setMaciParameters(...maciParameters.values())).to.be.revertedWith(
  ...
  it("deploys MACI", async () => {
    ...
    expect(await getGasUsage(deployTx)).lessThan(10000000); // origin: 7200000
  });
